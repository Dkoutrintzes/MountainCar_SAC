...
class SACAgent:
    ...
    def __init__(self, environment):
        ...
        self.critic_local2 = Network(input_dimension=self.state_dim,
                                     output_dimension=self.action_dim)
        self.critic_local2 = self.critic_local2.cuda()                             
    ...
    def train_networks(self, transition):
        ...
        if self.replay_buffer.get_size() >= self.REPLAY_BUFFER_BATCH_SIZE:
            ...
            rewards_tensor = torch.tensor(np.array(minibatch_separated[2])).to(device).float()
    ...
    def get_action_probabilities(self, state):
        ...
        action_probabilities = self.actor_local.forward(state_tensor.to(device))
